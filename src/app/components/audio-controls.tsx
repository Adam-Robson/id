...
export default function AudioControls({ onToggleList, listOpen, onPrev, onNext, minimized, onExpand }: AudioControlsProps) {
  const audioContext = useAudio();

  if (!audioContext) return null;

  const { isPlaying, togglePlay, seek, progress, duration, fmt, songs, current } = audioContext;
    ...
    >
      ...
      <div className="controls">
        ...
        <div className="playback-controls">
          <button className="ctrl-btn" onClick={onPrev} aria-label="Previous song">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/>
            </svg>
          </button>
          <button className="ctrl-btn ctrl-btn--play" onClick={e => { e.stopPropagation(); togglePlay(); }} aria-label={isPlaying ? 'Pause' : 'Play'}>
            {isPlaying ? (
          ...
          <button className="ctrl-btn" onClick={onNext} aria-label="Next song">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
